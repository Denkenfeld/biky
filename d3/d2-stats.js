...
function d2RenderMainChart(scope, rides){
  ...
  if(!canvas) return;
  // Ensure canvas has dimensions before creating gradient (may be 0 on first paint)
  const chartH = canvas.offsetHeight || canvas.parentElement?.offsetHeight || 140;
  ...
  if(existing){
    // Preserve canvas pixel size before any manipulation
    const w = canvas.offsetWidth, h = canvas.offsetHeight;
    try{ existing.destroy(); }catch(e){}
    // Restore size so Chart.js doesn't render into a 0×0 canvas
    canvas.style.width  = w ? w+'px' : '';
    canvas.style.height = h ? h+'px' : '';
  }
