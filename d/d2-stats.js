...
.d2-heatmap-scroll-wrap{overflow-x:auto;padding-bottom:4px;}
...
.d2-radial-month-label{font-size:8px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;fill:var(--muted);}
.d2-radial-center-text{font-family:var(--mono);font-size:11px;font-weight:700;fill:var(--green);text-anchor:middle;dominant-baseline:middle;}
.d2-radial-center-sub{font-size:8px;fill:var(--muted);text-anchor:middle;dominant-baseline:middle;}
...
function d2RenderHeatmap(scope, allRides){
  ...
  // Scrollbarer Wrapper für Monate + Grid zusammen
  const scrollWrap=document.createElement('div'); scrollWrap.className='d2-heatmap-scroll-wrap';
  ...
  const od=document.createElement('div'); od.style.overflowX='visible';
  ...
  od.appendChild(grid); inner.appendChild(od); scrollWrap.appendChild(inner); container.appendChild(scrollWrap);
...
function d2RenderRadialHeatmap(scope, allRides){
  ...
  const cx=140,cy=140,r0=28,r1=124,NS='http://www.w3.org/2000/svg';
  const svg=document.createElementNS(NS,'svg');
  svg.setAttribute('viewBox','0 0 280 280');
  svg.setAttribute('class','d2-radial-svg');

  // Ring pro Tag (365/366 Segmente)
  const anglePerDay = (2*Math.PI)/totalDays;
  const gap = 0.008; // Lücke zwischen Segmenten in rad
  ...
  for(let d=0;d<totalDays;d++){
    ...
    const x0i=cx+r0*cos0,y0i=cy+r0*sin0,x1i=cx+r0*cos1,y1i=cy+r0*sin1;
    const x0o=cx+r1*cos0,y0o=cy+r1*sin0,x1o=cx+r1*cos1,y1o=cy+r1*sin1;
    const lg=anglePerDay>Math.PI?1:0;
    const path=document.createElementNS(NS,'path');
    path.setAttribute('d',`M${x0i},${y0i} L${x0o},${y0o} A${r1},${r1},0,${lg},1,${x1o},${y1o} L${x1i},${y1i} A${r0},${r0},0,${lg},0,${x0i},${y0i}Z`);
    ...
    path.setAttribute('data-date',ds);
    path.setAttribute('data-cnt',cnt);
    path.style.cursor='default';
    svg.appendChild(path);
  }

  // Monatslinien und Beschriftungen
  const mStartAngles=[];
  for(let m=0;m<12;m++){
    const dayOffset=(new Date(year,m,1)-new Date(year,0,1))/86400000;
    mStartAngles.push(dayOffset);
  }
  mStartAngles.forEach((dayOff,m)=>{
    const angle=-Math.PI/2+dayOff*anglePerDay;
    const x0=cx+(r0-2)*Math.cos(angle),y0=cy+(r0-2)*Math.sin(angle);
    const x1=cx+(r1+2)*Math.cos(angle),y1=cy+(r1+2)*Math.sin(angle);
    const line=document.createElementNS(NS,'line');
    line.setAttribute('x1',x0);line.setAttribute('y1',y0);line.setAttribute('x2',x1);line.setAttribute('y2',y1);
    line.setAttribute('stroke','rgba(255,255,255,0.15)');line.setAttribute('stroke-width','0.8');
    svg.appendChild(line);

    // Monatsbeschriftung mittig zwischen zwei Linien
    const nextOff=m<11?mStartAngles[m+1]:totalDays;
    const midDay=(dayOff+nextOff)/2;
    const ma=-Math.PI/2+midDay*anglePerDay;
    const labelR=r1+12;
    ...
    // Rotation damit Text radial ausgerichtet ist
    const deg=(ma+Math.PI/2)*(180/Math.PI);
    txt.setAttribute('transform',`rotate(${deg},${lx},${ly})`);
    txt.textContent=mns[m];
    svg.appendChild(txt);
  });

  // Wochentag-Ringe (leichte Linien)
