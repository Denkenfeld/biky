...
function d2RenderHeatmap(scope, allRides){
  ...
  const totalDays=(new Date(year,11,31)-sd)/86400000+1;
  ...
  const DAY_LABEL_W = 18; // px reserved for Mo/Mi/Fr labels
  const availW = container.clientWidth - DAY_LABEL_W;
  // gap between cells is 2px; total = weeks*cw + (weeks-1)*gap
  // solve: weeks*cw + (weeks-1)*2 = availW  →  cw = (availW + 2) / weeks - 2
  const gap = 2;
  const cw = Math.max(4, Math.floor((availW + gap) / totalWeeks - gap));
  const ch = cw; // square cells

  container.innerHTML='';

  // Month labels row — positioned by column index
  const mr=document.createElement('div');
  mr.style.cssText=`display:flex;gap:0;margin-bottom:3px;padding-left:${DAY_LABEL_W}px;font-size:7px;font-weight:700;letter-spacing:.05em;color:var(--muted);text-transform:uppercase;overflow:hidden;`;
  let prevCol=0;
  mpos.forEach(mp=>{
    const gap_cols = mp.col - prevCol;
    const sp=document.createElement('div');
    sp.style.cssText=`flex-shrink:0;width:${gap_cols*(cw+gap)}px;min-width:0;`;
    mr.appendChild(sp);
    const lb=document.createElement('div');
    lb.textContent = cw >= 7 ? mp.name : mp.name.slice(0,1); // single letter when very small
    lb.style.cssText='flex-shrink:0;overflow:hidden;white-space:nowrap;';
    mr.appendChild(lb);
    prevCol = mp.col + 1;
  });
  ...
  dl.style.cssText=`display:flex;flex-direction:column;gap:${gap}px;margin-right:${gap}px;flex-shrink:0;width:${DAY_LABEL_W-gap}px;`;
  ['Mo','','Mi','','Fr','','So'].forEach(d=>{
    const s=document.createElement('span');
    s.textContent=d;
    s.style.cssText=`font-size:7px;color:var(--muted);height:${ch}px;display:flex;align-items:center;font-family:var(--mono);overflow:hidden;`;
    dl.appendChild(s);
  });
  inner.appendChild(dl);

  // SVG-based grid — one rect per day, far faster than 365 divs and scales perfectly
  const svgW = totalWeeks*(cw+gap)-gap;
  const svgH = 7*(ch+gap)-gap;
  const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
  svg.setAttribute('width', svgW);
  svg.setAttribute('height', svgH);
  svg.style.cssText='display:block;flex-shrink:0;overflow:visible;';

  const levelColors=['rgba(255,255,255,0.04)','rgba(0,230,118,0.2)','rgba(0,230,118,0.45)','rgba(0,230,118,0.7)','rgba(0,230,118,0.95)'];

  for(let i=0;i<totalCells;i++){
    const do2=i-offset;
    const col=Math.floor(i/7), row=i%7;
    const x=col*(cw+gap), y=row*(ch+gap);
    if(do2<0||do2>=totalDays) continue;
    const dt=new Date(year,0,1+do2); const ds=dt.toISOString().slice(0,10);
    const cnt=dc[ds]||0;
    const lvl=cnt>0?Math.ceil((cnt/maxC)*4):0;
    const rect=document.createElementNS('http://www.w3.org/2000/svg','rect');
    rect.setAttribute('x',x); rect.setAttribute('y',y);
    rect.setAttribute('width',cw); rect.setAttribute('height',ch);
    rect.setAttribute('rx',Math.max(1,Math.round(cw*0.2)));
    rect.setAttribute('fill',levelColors[lvl]);
    if(cnt>0) rect.style.filter='drop-shadow(0 0 2px rgba(0,230,118,0.3))';
    rect.innerHTML=`<title>${ds}${cnt>0?`: ${cnt} Tour${cnt>1?'en':''}`:''}`;
