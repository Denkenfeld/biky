...
function d2RenderDoughnut(scope, rides){
  ...
  // Force pixel dimensions directly — works even inside display:none containers
  canvas.width = 110; canvas.height = 110;
  const ch = new Chart(canvas.getContext('2d'), {
    type:'doughnut',
    data:{ labels:['<30km','30–70km','>70km'], datasets:[{ data:[s,m,l],
      backgroundColor:['rgba(0,230,118,0.75)','rgba(0,212,255,0.7)','rgba(255,145,0,0.75)'],
      borderColor:['rgba(0,230,118,1)','rgba(0,212,255,1)','rgba(255,145,0,1)'],
      borderWidth:1.5, hoverOffset:4 }] },
    options:{ responsive:false, cutout:'66%',
      plugins:{ legend:{display:false},
        tooltip:{ callbacks:{label:c=>`${c.label}: ${c.raw}`},
          backgroundColor:'rgba(4,5,8,0.96)', borderColor:'rgba(255,255,255,0.12)',
          borderWidth:1, bodyFont:{family:'JetBrains Mono'} } }
    }
  });
  _d2Charts.set(canvas, ch);
}
