...
function d2RenderMainChart(scope, rides){
  ...
  // gradient plugin — recomputed on every draw so resize/tab-switch always works
  ...
  if(existing){
    ...
    existing.options.plugins.tooltip.callbacks.label = c=>`${c.raw.toFixed(_d2Metric==='km'?1:0)} ${_d2Metric==='km'?'km':_d2Metric==='hm'?'m':'kcal'}`;
  ...
  _d2Charts.set(canvas, new Chart(ctx, {
    ...
    data:{ labels, datasets:[{ data:dp, backgroundColor:'rgba(0,212,255,0.2)', borderColor:dp.map(v=>v>0?col+'1)':'rgba(255,255,255,0.07)'), borderWidth:1.5, borderRadius:3, borderSkipped:false }] },
